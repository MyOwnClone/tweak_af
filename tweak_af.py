...
def reload_dict(call_filename):
    ...
    for line in content:
        ...
        while True:
            ...
            call_hash = (line_counter+1) * 1000 + index

            # print(f"_tv() found at {call_filename}:{call_hash}, {line}")
            # print(f"_tv() found at {call_filename}:{line_counter+1}:{found_on_line_counter} == {value}")
...
def resolve_value(default_value, call_filename, line, line_order):
    ...
    if call_filename in tweak_dict and line in tweak_dict[call_filename] and line_order in tweak_dict[call_filename][line]:
